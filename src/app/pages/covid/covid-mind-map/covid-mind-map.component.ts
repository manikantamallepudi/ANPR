...
import * as Data from '../../../../assets/data/diagram.json';
@Component({
  selector: 'app-covid-mind-map',
  templateUrl: './covid-mind-map.component.html',
  styleUrls: ['./covid-mind-map.component.scss']
})
export class CovidMindMapComponent implements OnInit {
  ...
  // public items: DataManager = new DataManager(this.userMeet);
  ...
  //     return ((node as Node).data as UserInfo).branch;
  //   }
  // };
  public layout: Object;
  ...
  private nodeDefaults(obj: NodeModel): NodeModel {
    ...
    let empInfo: UserInfo = obj.data as UserInfo;
  ...
  private getConnectorDefaults(connector: ConnectorModel, diagram: Diagram): ConnectorModel {
    ...
    let empInfo: UserInfo = targetNode.data as UserInfo;
  ...
  public selectionChange(arg: ISelectionChangeEventArgs): void {
    if (arg.state === 'Changing') {
      if (arg.newValue[0] instanceof Node) {
        let empInfo: UserInfo = ((arg.newValue[0] as Node).data as UserInfo);
  ...
  ngOnInit() {
    console.log(this.userMeet);
    this.items = new DataManager(this.userMeet);
    this.data = { id: 'id', parentId: 'parentId', dataSource: this.items, root: '1' };
    this.layout = {
      type: 'MindMap', horizontalSpacing: 50,
      getBranch: (node: NodeModel, nodes: NodeModel[]) => {
        return ((node as Node).data as UserInfo).branch;
...
function addNode(): NodeModel {
  ...
  obj.id = randomId();
  obj.data = {};
  (obj.data as UserInfo).Label = 'Node';
  return obj;
}
    ...
    if (this.inAction) {
      ...
      if (selectedObject[0]) {
        if (selectedObject[0] instanceof Node) {
          ...
          let empInfo: UserInfo = (selectedObject[0].data as UserInfo);
          if (empInfo.branch === 'Root') {
            (node.data as UserInfo).branch = 'Right';
          } else if (empInfo.branch === 'Right' || empInfo.branch === 'subRight') {
            (node.data as UserInfo).branch = 'subRight';
          }
    ...
    if (this.inAction) {
      ...
      if (selectedObject[0]) {
        if (selectedObject[0] instanceof Node) {
          ...
          let empInfo: UserInfo = (selectedObject[0].data as UserInfo);
          if (empInfo.branch === 'Root') {
            (node.data as UserInfo).branch = 'Left';
          } else if (empInfo.branch === 'Left' || empInfo.branch === 'subLeft') {
            (node.data as UserInfo).branch = 'subLeft';
          }
          let connector: ConnectorModel = addConnector(selectedObject[0], node);
          this.diagram.clearSelection();
          let nd: Node = this.diagram.add(node) as Node;
